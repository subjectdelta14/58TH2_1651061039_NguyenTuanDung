...
window.onload = function(){
    ...
    let componentStyleHtml = '.component_mask{' +
      ...
      '.component_toast {' +
      'position: relative;top: 30%;margin: 0 auto;min-width: 200px;width: 300px;' +
      ...
      '.component_dialog_bd {' +
      'padding: 0 20px;font-size: 15px;color: black;word-wrap: break-word;word-break: break-all;}' +
      '.component_dialog_ft {' +
...
ConfirmClass.prototype.show =  function (options) {
  ...
  this.btns.forEach(function (v, i){
    btnHtml += '<a class="component_btn '+ (v.class||'') +'" style="cursor: pointer;" v-on:click="ConfirmClass.btnClick('+ i +', event)">'+ (v.text||'Có') +'</a>'
  });
