...
export default {
  ...
  methods: {
    ...
    async getProduct() {
      this.oneProduct = await ApiFunction("qlvl/acc/" + this.id, "GET", null);
      this.userName = this.oneProduct.username;
      this.passWord = this.oneProduct.password;
      this.role = this.oneProduct.role;
  ...
  UpdateAccount() {
      ...
      if (self.userName == "") {
        ...
        ApiFunction("qlvl/acc/"+this.id, "POST", newsModel);
      }
    },
  },
  created() {
